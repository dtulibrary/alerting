...
describe AlertRunner do
  ...
  before do
    ...
    stub_request(:get, /#{Rails.application.config.solr[:url]}.*/).with(:query => hash_including({"fl" => "alert_timestamp_dt"})).to_return(:body => "{'response'=>{'numFound'=>248723844,'start'=>0,'docs'=>[{'alert_timestamp_dt'=>'#{DateTime.current - 1.day}'}]}}")
